...
//		LeftHandCursor,
		...
		PointRight,        // Point gesture with right hand
		PointLeft,         // Point gesture with left hand
...
//							}
			...
			// check for PointRight - arm extended to the right with elbow straight
			case Gestures.PointRight:
				switch(gestureData.state)
				{
					case 0:  // gesture detection
						if(jointsTracked[rightHandIndex] && jointsTracked[rightElbowIndex] && jointsTracked[rightShoulderIndex] &&
							// Hand should be extended to the right and forward
							(jointsPos[rightHandIndex].x - jointsPos[rightShoulderIndex].x) > 0.2f &&
							// Hand should be at roughly shoulder height
							Mathf.Abs(jointsPos[rightHandIndex].y - jointsPos[rightShoulderIndex].y) < 0.2f &&
							// Elbow and hand should be roughly aligned (arm extended)
							Mathf.Abs(jointsPos[rightHandIndex].x - jointsPos[rightElbowIndex].x) > 0.1f)
						{
							SetGestureJoint(ref gestureData, timestamp, rightHandIndex, jointsPos[rightHandIndex]);
						}
						break;
			...
			// check for PointLeft - arm extended to the left with elbow straight
			case Gestures.PointLeft:
				switch(gestureData.state)
				{
					case 0:  // gesture detection
						if(jointsTracked[leftHandIndex] && jointsTracked[leftElbowIndex] && jointsTracked[leftShoulderIndex] &&
							// Hand should be extended to the left and forward
							...
							// Elbow and hand should be roughly aligned (arm extended)
							Mathf.Abs(jointsPos[leftHandIndex].x - jointsPos[leftElbowIndex].x) > 0.1f)
						{
							SetGestureJoint(ref gestureData, timestamp, leftHandIndex, jointsPos[leftHandIndex]);
						}
						break;
